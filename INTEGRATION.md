...
платежи через ЮKassa и автоматически выдавать файл из Google Drive (или ссылку
на Notion / Google Sheets) на e‑mail покупателя. Всё работает
...
[n8n: webhook /delaydelo-create-payment]   (CORS под delo-delai.ru)
        ...
        │  3) ЮKassa API: создаём платёж + metadata.project = "delaydelo"
...
[n8n: receiver Делай Дело] /webhook/delaydelo-yukassa
        ...
        ▼
[Покупатель: получает файл/ссылку]
```
...
2. **Интеграция → HTTP-уведомления** → добавьте один webhook на события платежа:
...
3. **Тест-режим магазина.** Новые магазины ЮKassa по умолчанию в test mode — это и нужно для первых тестовых платежей картой `5555 5555 5555 4477`. После успешных тестов переключите магазин в production.
4. **Лимиты НПД (самозанятый):**
...
5. На указанный email должно прийти письмо с файлом или ссылкой (через workflow `delaydelo-yukassa-receiver`).
6. Перейдёте на страницу `/thanks/<slug>/` после оплаты.

Если файл или ссылка не пришли — проверьте n8n executions у обоих workflow: видны все шаги и любые ошибки валидации (например, неверный slug).
